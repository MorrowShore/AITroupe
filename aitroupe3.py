...
async def on_ready():
    ...
    print(f'\n⚠️ If it doesnt work, re-invite your bot using the link that has the correct permission integar.\n')
...
async def on_message(message):
    global conversation_history, last_human_message, human_reply_direct, human_reply_recent, CURRENT_AI, CHANNEL_ID, thread_histories
    ...
    if not message.author.bot:
        ...
        if mentioned_personas:
            if not THREAD_MODE:
                print(f"  ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ")
                print(f" 🔤 Generating Direct Response - Direct={human_reply_direct}, Recent={human_reply_recent}, ThreadMode={THREAD_MODE}")
            
            for i, persona in enumerate(mentioned_personas):
                try:
                    # Create thread if thread mode is enabled
                    if THREAD_MODE and i == 0:  # Only create thread for first persona
                        thread_name = f"{persona['name']}'s conversation with {message.author.display_name}"
                        ...
                        await generate_persona_response_in_thread(persona, thread, human_reply_recent)
                        continue
                    
                    # Normal main channel response (when thread mode off or additional personas)
                    if not THREAD_MODE or i > 0:
                        ...
                        print(f"🗿 Giving response to Human  (direct)")
...
@bot.command(name='threadmode')
@is_admin_or_owner()
async def toggle_threadmode(ctx):
    """Toggle thread mode on/off (admin only)"""
    global THREAD_MODE
    THREAD_MODE = not THREAD_MODE
    status = "ON" if THREAD_MODE else "OFF"
    await ctx.send(f"Thread mode: **{status}**")
...
@bot.tree.command(name="aitroupe_status", description="Show current AI provider, channel, and thread mode")
async def slash_status(interaction: discord.Interaction):
    """Show current AI provider, channel, and thread mode"""
    if not (interaction.user.guild_permissions.administrator or await bot.is_owner(interaction.user)):
        await interaction.response.send_message("You need administrator permissions to use this command.", ephemeral=True)
        return
    ...
    thread_status = "ON" if THREAD_MODE else "OFF"
    await interaction.response.send_message(f"Current AI provider: **{CURRENT_AI}**\nActive channel: {channel_status}\nThread mode: **{thread_status}**", ephemeral=True)


    


@bot.tree.command(name="aitroupe_threadmode", description="Toggle thread mode on/off")
async def slash_threadmode(interaction: discord.Interaction):
    """Toggle thread mode on/off"""
    if not (interaction.user.guild_permissions.administrator or await bot.is_owner(interaction.user)):
        await interaction.response.send_message("You need administrator permissions to use this command.", ephemeral=True)
        return
    ...
    THREAD_MODE = not THREAD_MODE
    status = "ON" if THREAD_MODE else "OFF"
    await interaction.response.send_message(f"Thread mode: **{status}**")
...
async def handle_thread_message(message):
    ...
    if thread_id not in thread_histories:
        return  # Thread not managed by bot
    ...
    # Keep thread history manageable
    thread_histories[thread_id] = thread_histories[thread_id][-CONVERSATION_HISTORY_LIMIT:]
    
    # Find which persona owns this thread (from thread name or history)
    thread_persona = None
    for persona in PERSONAS:
        if persona['name'].lower() in message.channel.name.lower():
            thread_persona = persona
            break
    
    if not thread_persona:
        return
    
    # Generate response from the thread's persona
    await generate_persona_response_in_thread(thread_persona, message.channel, True)


async def generate_persona_response_in_thread(persona, thread, human_reply_recent):
    """Generate persona response in a thread"""
    global thread_histories
    ...
    try:
        print(f"   ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~")
        print(f"   🔤 Generating Thread Response - Direct=True, Recent={human_reply_recent}, Thread=True")
        
        # Use thread-specific history for context
        thread_history = thread_histories[thread_id]
        context_messages = [clean_emotes_from_conversation(msg["content"]) for msg in thread_history[-CONTEXT_MESSAGES_LIMIT:]]
        context = "\n".join(context_messages)
        
        # Build the complete prompt here - don't let get_ai_response rebuild it
        prompt = (
            f"You are {persona['name']}, {persona['role']}. {HUMAN_PROMPT}\n\n"
            f"Current conversation:\n{context}\n\n"
        )
        
        # Pass the pre-built prompt and indicate we don't want it rebuilt
        reply = await get_ai_response(persona, prompt, human_reply_recent=False, thread_history=None, use_provided_prompt=True)
        
        if reply:
            print(f"   📏 Raw AI response length: {len(reply)} chars")
            print(f"   🗿 Giving response to Human (thread)")
        ...
        if reply and not reply.isspace():
            ...
            if reply:
                print(f"   🚩 Flags - Direct=False, Recent={human_reply_recent}, Thread=True")
                print(f"   📝 Final response length: {len(reply)} chars")
                
                # Add to thread history
                ai_message = {"role": "assistant", "content": f"{persona['name']}: {reply}"}
                thread_histories[thread_id].append(ai_message)
                thread_histories[thread_id] = thread_histories[thread_id][-CONVERSATION_HISTORY_LIMIT:]
                
                # Send message in thread using webhook
                await send_message_as_persona_in_thread(persona['name'], reply, thread)
                
    except Exception as e:
        print(f"   🚫 Error generating thread response for {persona['name']}: {e}")


async def send_message_as_persona_in_thread(persona_name, message, thread):
    """Send a message as persona in a thread"""
    try:
        if persona_name in persona_webhooks:
            webhook = persona_webhooks[persona_name]
        ...
        print(f"   🚫 Error sending thread message as {persona_name}: {e}")
...
async def get_ai_response(persona, prompt, human_reply_recent=False, thread_history=None, use_provided_prompt=False):
    """Get response from the current AI provider"""
    global CURRENT_AI
    
    # If use_provided_prompt is True, skip the context building and use the prompt as-is
    if not use_provided_prompt:
        ...
        if human_reply_recent or is_technical:
            # Use thread_history if provided, otherwise use global conversation_history
            if thread_history is not None:
                # Use thread-specific history
                context_messages = [clean_emotes_from_conversation(msg["content"]) for msg in thread_history[-CONTEXT_MESSAGES_LIMIT:]]
            else:
...
async def conversation_loop(channel):
    ...
    try:
        ...
        if reply and not reply.isspace():
            ...
            if human_reply_recent:
                reply = reply.strip()
                print(f"🗿 Giving response to Human  (main channel)")
            else:
                reply = reply.split('\n')[0].strip()  # Take first line only
                print(f"🤖 Giving response to AI  (main channel)")
